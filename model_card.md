...
**New limitation introduced by the trained model:** its training labels are synthetic — generated from a documented "teacher policy" (see Section 3) that mimics the original hand-coded formula, not from real listener feedback. So the model has learned to reproduce a human-authored scoring rule slightly more flexibly; it hasn't learned anything about music taste that wasn't already encoded in that rule. **This was tested, not just proposed** — `src/train_model_real.py` trains a second model on 18 real liked/skipped labels (`data/real_feedback.csv`), and `python -m src.demo_real_vs_synthetic` compares it directly against the synthetic model (see README's "Optional: Train on Real Feedback" section for the full output). The result: the real model's learned coefficients look nothing like the synthetic one's. `genre_similarity` came out near `0` (genre didn't predict what this listener actually picked), `energy_closeness` came out *negative* (matching the stated energy target didn't make a song more likely to be liked), and `mood_similarity` was the one feature that meaningfully mattered. In other words, the synthetic policy encodes what the *assignment's original formula assumed* mattered — it is not a substitute for what a real listener actually responds to, and 18 labels is too little data to fit 4 features reliably (validation MAE `0.499` vs. the synthetic model's `0.042`). That gap is the real limitation of a synthetic-label approach, made concrete instead of hypothetical.
...
There was also a section where the AI made its own suggestion about documenting how different the models I trained and the synthetically trained models differed. It noted how I had preferences to mood, while energy or genre barely had any impact on my rankings.

"This is a genuinely interesting result worth documenting properly — the real-feedback model learned something structurally different from the synthetic one (mood mattered, energy target didn't, genre barely registered). Let me capture this in the docs."
...
## 11. Reflection and Ethics: Thinking Critically About Your AI
...
**describe your collaboration with AI during this project. Identify one instance when the AI gave a helpful suggestion and one instance where its suggestion was flawed or incorrect.** The AI gave helpful suggestions when talking about what models to use when calculating recommendation values for songs. It was not helpful when it tried to create an entire LLM model, requesting gemini API keys, and tried to build a real system.
